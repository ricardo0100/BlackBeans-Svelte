<script>
  ...
  import { getAccounts, getCategories } from "../../../../util/API";
  ...
  <div class="dropdown">
    <button class="btn btn-outline-secondary dropdown-toggle p-2" type="button" id="dropdownAccounts" data-bs-toggle="dropdown" aria-expanded="false">
      {#if filter.selectedCategory == null}
        All categories
      {:else}
        <span class="material-icons me-2" style="color: {filter.selectedCategory.color}; font-size: 1rem;">{filter.selectedCategory.icon}</span>{filter.selectedCategory.name}
      {/if}
    </button>
    <ul class="dropdown-menu" aria-labelledby="dropdownAccounts">
      <li>
